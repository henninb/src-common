...
function setMenu(d, m, y, h, min) {
 if (d>0)
     document.myform.Tag.options[d-1].selected=true; else alert(d);
 if (m>0)
     document.myform.Monat.options[m-1].selected=true; else alert(m);
...
function currentTime() {
 ...
 if (curOffset>=1320) curOffset=curOffset-1440;
...
//alert(dat.toString())
 ...
 if (hours>=24) {hours=hours-24;day=day+1;}
 daysInMonth(month,year);
 if (day>dIM) {day=1;month=month+1;}
 if (month>12) {month=1;year=year+1;}

 if (hours<0) {hours=hours+24;day=day-1;}
 daysInMonth(month,year);
 if (day<1) {day=dIM;month=month-1;}
 if (month<1) {month=1;year=year-1;}

utHours = hours - locOffset;

 if (utHours>=24) {utHours=utHours-24;utDay=utDay+1;}
 if (utDay>dIM) {utDay=1;utMonth=utMonth+1;}
 if (utMonth>12) {utMonth=1;utYear=utYear+1;}

 if (utHours<0) {utHours=utHours+24;utDay=utDay-1;}
 if (utDay<1) {utDay=dIM;utMonth=utMonth-1;}
 if (utMonth<1) {utMonth=1;utYear=utYear-1;}
...
function getLocation(LatLong) {
 ...
 if (doffset>=1320) doffset=doffset-1440;
...
var dat = new Date(Date.UTC(year,month-1,utDay,utHours,minutes,seconds));
 ...
 if (theOffset>=1320) theOffset=theOffset-1440;
...
function calculate() {
 ...
 var dat1 = new Date();
 seconds = dat1.getSeconds();
 if (seconds==0) {minutes=minutes+1; if (minutes==60)
 {minutes=0;
 hours=hours+1;
 utHours=utHours+1;
 if (hours==24) {hours=0;day=day+1;}
 if (utHours==24) {utHours=0;utDay=utDay+1;}
 daysInMonth(month,year);
 if (day>dIM) {day=1;month=month+1;}
 if (utDay>dIM) {utDay=1;utMonth=utMonth+1;}
 if (month>12) {month=1;year=year+1;}
 ...
 if (document.myform.EastWest.selectedIndex==1) longit=-Math.abs(longit);
...
function GM_Sidereal_Time( jd ) {
  ...
  //document.write("<br>jd=" + jd + "<br>");
  //document.write("<br>UT=" + UT + "<br>");
  MJD = jd - 2400000.5;
  MJD0 = Math.floor(MJD);
  ut = (MJD - MJD0)*24.0;
